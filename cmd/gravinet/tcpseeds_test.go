...
// no-port-uses-default-tcp-port and explicit-port-used-verbatim cases
...
func TestResolveTCPSeedsBasics(t *testing.T) {
	...
	if got := resolveTCPSeeds([]string{"203.0.113.9:60000"}, config.DefaultTCPPort, nil); len(got) != 0 {
		t.Errorf("bare udp-style seed leaked into TCP resolution: %v", got)
	}

	// No port → the default TCP port.
	...
	got = resolveTCPSeeds([]string{"tcp://203.0.113.9:60000"}, config.DefaultTCPPort, nil)
	if len(got) != 1 || got[0].Port() != 60000 {
		t.Fatalf("explicit tcp port: got %v, want one endpoint on port 60000", got)
	}

	// Duplicates collapse.
	if got := resolveTCPSeeds([]string{"tcp://203.0.113.9:443", "tcp://203.0.113.9:443"}, config.DefaultTCPPort, nil); len(got) != 1 {
...
func TestResolveTCPSeedsMultiPort(t *testing.T) {
	got := resolveTCPSeeds([]string{"tcp://203.0.113.9:60000,60001,60002"}, config.DefaultTCPPort, nil)
	if len(got) != 3 {
...
func TestResolveTCPSeedsRejectsOverlayAddress(t *testing.T) {
	...
	got := resolveTCPSeeds([]string{"tcp://10.77.42.9:51820"}, config.DefaultTCPPort, []netip.Prefix{overlay})
	if len(got) != 0 {
		t.Fatalf("expected overlay tcp seed to be rejected, got %v", got)
	}
	got = resolveTCPSeeds([]string{"tcp://198.51.100.7:51820"}, config.DefaultTCPPort, []netip.Prefix{overlay})
	if len(got) != 1 {
