...
// TestConfigExposesSNMPAndLLDPSupported checks that /api/config carries
// the capability flags the web UI's sectionVisible() gates System > SNMP,
// System > LLDP, and System > Syslog on — the same "menu the user sees
...
// this test suite.
func TestConfigExposesSNMPAndLLDPSupported(t *testing.T) {
	dir := t.TempDir()
	...
	wantLLDP, _ := service.LLDPSupported()
	if got, ok := out["lldp_supported"].(bool); !ok || got != wantLLDP {
		t.Errorf("lldp_supported = %#v, want %v", out["lldp_supported"], wantLLDP)
	}
