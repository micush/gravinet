...
func TestQoSRuleSetEnabled(t *testing.T) {
	...
	if err := c.QoSRuleSetEnabled("tcp", 22, nil, "", false); err != nil {
		t.Fatal(err)
	}
	if len(c.QoS.Rules) != 1 || !c.QoS.Rules[0].Disabled {
		t.Fatalf("rule should be present and disabled: %+v", c.QoS.Rules)
	}
	// Re-enable it.
	if err := c.QoSRuleSetEnabled("tcp", 22, nil, "", true); err != nil {
		t.Fatal(err)
	}
	if c.QoS.Rules[0].Disabled {
	...
	if err := c.QoSRuleSetEnabled("udp", 9999, nil, "", false); err == nil {
...
func TestQoSRuleServices(t *testing.T) {
	c := &Config{UDPPorts: []int{65432}, EnableIPv4: true,
		Networks: []Network{{ID: "1234", Name: "lan", Enabled: true, Subnet4: "10.0.0.0/24"}}}
	if err := c.QoSAdd("", 0, []string{"ssh", "rdp"}, 0, ""); err != nil {
		t.Fatal(err)
	}
	if got := c.QoS.Rules[0].Services; len(got) != 2 {
		t.Fatalf("services = %v, want [ssh rdp]", got)
	}
	// Keyed order- and case-insensitively.
	if err := c.QoSRuleSetEnabled("", 0, []string{"RDP", "SSH"}, "", false); err != nil {
	...
	if err := c.QoSRuleSetEnabled("", 0, []string{"dns"}, "", true); err == nil {
		t.Error("toggling a rule by an unrelated service set should error")
	}
	if err := c.QoSDelete("", 0, []string{"ssh", "rdp"}, ""); err != nil {
		t.Fatal(err)
	}
	if len(c.QoS.Rules) != 0 {
		t.Fatalf("rule should be gone: %+v", c.QoS.Rules)
	}
}
