...
// Syslog page. Points this host's *local* syslog daemon at a remote
// collector; deliberately additive, never a replacement — whatever this
// host already logs locally (its distro's default syslog.conf/rsyslog.conf
// rules) keeps landing wherever it already lands. gravinet only ever adds
// or removes its own clearly-marked forwarding rule alongside that, the
// same non-destructive "rewrite only our own delimited block, leave
// everything else exactly as it was" shape package hosts already uses for
// the OS hosts file.
//
...
// could silently win on the next reload.
//
// Supported:
//
...
//	         syntax ("@host" for UDP, "@@host" for TCP) — a managed block
//	         appended to the existing file, mirroring hosts.Render.
...
type SyslogInfo struct {
	Enabled   bool   // is a remote forward currently configured
	Target    string // "host:port" of the configured remote collector, or ""
	Protocol  string // "udp" or "tcp"; "" when Enabled is false
...
func HostSyslog() SyslogInfo {
	...
	case "linux":
		...
		if target, proto, ok := parseRsyslogDropIn(); ok {
			info.Enabled, info.Target, info.Protocol = true, target, proto
		}
	case "freebsd", "openbsd":
		info.Manager = "syslogd"
		if target, proto, ok := parseBSDSyslogBlock(syslogConfPath); ok {
			info.Enabled, info.Target, info.Protocol = true, target, proto
		}
	}
	return info
...
// SetHostSyslog enables or disables forwarding this host's syslog to a
// remote collector. Disabling removes only gravinet's own managed
// drop-in/block — never anything else in the host's syslog config, on any
// platform this supports (see the package comment for exactly which).
func SetHostSyslog(enabled bool, target, protocol string) (bool, string) {
	...
	if enabled {
		...
		if err := validSyslogTarget(target); err != nil {
	...
	switch runtime.GOOS {
	case "linux":
		return setLinuxSyslog(enabled, target, protocol)
	default: // freebsd, openbsd — the only other case SyslogSupported() lets through
		return setBSDSyslog(enabled, target, protocol)
	}
}
...
// the file's mere presence/absence *is* whether forwarding is on.
const rsyslogDropIn = "/etc/rsyslog.d/60-gravinet-syslog-forward.conf"

func setLinuxSyslog(enabled bool, target, protocol string) (bool, string) {
	if !enabled {
		if err := os.Remove(rsyslogDropIn); err != nil && !os.IsNotExist(err) {
			return false, "couldn't remove " + rsyslogDropIn + ": " + err.Error()
		}
	} else {
		host, port, _ := net.SplitHostPort(target) // shape already validated by SetHostSyslog
		...
		if err := writeFilePreserving(rsyslogDropIn, []byte(renderRsyslogDropIn(host, port, protocol)), 0o644); err != nil {
...
// renderRsyslogDropIn renders rsyslogDropIn's content for a given
// host/port/protocol — the RainerScript action() syntax rsyslog's omfwd
// module reads for a forwarding rule.
func renderRsyslogDropIn(host, port, protocol string) string {
	var b strings.Builder
	b.WriteString("# Managed by gravinet (System > Syslog). Local logging is untouched;\n")
	b.WriteString("# this file only adds a forwarding copy alongside it. Disabling\n")
	b.WriteString("# forwarding in gravinet (or deleting this file) removes only that.\n")
	fmt.Fprintf(&b, "*.* action(type=%q target=%q port=%q protocol=%q)\n", "omfwd", host, port, protocol)
	return b.String()
}

// parseRsyslogDropIn reads back the target/protocol gravinet itself last
// wrote to rsyslogDropIn.
func parseRsyslogDropIn() (target, protocol string, ok bool) {
	return parseRsyslogDropInAt(rsyslogDropIn)
}

// parseRsyslogDropInAt is parseRsyslogDropIn with the path pulled out, so
// the parsing logic is testable against a temp file rather than the real
// /etc/rsyslog.d. It only ever needs to understand output this same code
// produced — the file is entirely gravinet's own — so this reads the three
// quoted action() attributes back out directly rather than parsing
// rsyslog's full action() grammar.
func parseRsyslogDropInAt(path string) (target, protocol string, ok bool) {
...
func setBSDSyslog(enabled bool, target, protocol string) (bool, string) {
	block := ""
	if enabled {
		block = renderBSDSyslogBlock(target, protocol)
	}
...
// /etc/syslog.conf. BSD syslogd's own remote-forward syntax: a selector
// (here "*.*", everything) followed by "@host" to forward over UDP or
// "@@host" over TCP — long-standing, documented behavior shared by both
// FreeBSD's and OpenBSD's syslogd.
func renderBSDSyslogBlock(target, protocol string) string {
	prefix := "@"
	if protocol == "tcp" {
		prefix = "@@"
	}
...
// parseBSDSyslogBlock reads back the target/protocol from the
// gravinet-managed block in path, or ok=false if there isn't one.
func parseBSDSyslogBlock(path string) (target, protocol string, ok bool) {
	b, err := os.ReadFile(path)
	if err != nil {
		return "", "", false
	}
	...
	for _, ln := range strings.Split(string(b), "\n") {
		...
		case strings.HasPrefix(fields[1], "@@"):
			return strings.TrimPrefix(fields[1], "@@"), "tcp", true
		case strings.HasPrefix(fields[1], "@"):
			return strings.TrimPrefix(fields[1], "@"), "udp", true
		}
	}
	return "", "", false
}
...
// validSyslogTarget requires "host:port". host may be a hostname or a
// literal IP address — the remote syslog daemon resolves it, gravinet
// doesn't need to. Reuses validHostname's character-set restriction for a
// non-IP host, for the same injection-safety reason validTimezone/
// validNTPServer apply it elsewhere in this package: whatever passes here
// is written straight into a daemon's own config file, and a directive
// value is just as capable of smuggling a second directive via a stray
// newline or quote as an exec.Command argument is of smuggling a second
// command.
func validSyslogTarget(s string) error {
	host, port, err := net.SplitHostPort(s)
	if err != nil {
		return fmt.Errorf(`target must be host:port, e.g. "log.example.com:514"`)
	}
