...
// TCP tests spell this loop out inline each time.
...
// watchTCPHandshake has recorded an escalating backoff against a TCP
...
// back: tcpInBackoff's only caller was primeTCPSeeds' explicit-TCP-seed
...
// tcpDialCooldown's flat 30s stayed the real pace, and the warning
...
func TestDialCandidateHonorsBackoff(t *testing.T) {
	...
	// watchTCPHandshake penalizes fb — reproduced directly rather than
...
// (watchTCPHandshake's success path) and the address becomes dialable
// again.
