...
// TestDeadSeedWithTCPDoesNotDegrade is
// TestDeadSeedRetryDoesNotDegradeOtherPeers's counterpart with real TCP/TLS
// TCP enabled on both nodes (production always enables this — see
// main.go's cfg.TCPEnabled()), so I's dead "gn-cush1" seed actually
// exercises ensureTCP -> Dual.DialTCP -> TLSTransport.Dial against
// a target with nothing listening on either UDP or TCP, not just the UDP
// path in isolation.
func TestDeadSeedWithTCPDoesNotDegrade(t *testing.T) {
	if testing.Short() {
	...
	mk := func(name string, self netip.Addr, tcpPort int) *node {
		...
		eng := NewEngine(Options{
			NodeID: name, Hostname: name, TCPPort: tcpPort,
	...
	// also point the dead seed's TCP attempt at deterministically.
	...
	// seed backs off on UDP, ensureTCP's real TLSTransport.Dial gets
	...
	for time.Now().Before(deadline) {
		...
		case <-time.After(5 * time.Second):
			close(drain)
			t.Fatalf("round %d (t+%s): I did not receive M's packet — reproduces the reported degradation with real TCP dialing active", round, testDuration-time.Until(deadline))
		}
		time.Sleep(pollEvery)
	}
	t.Logf("completed %d rounds of M->I delivery over %s with real TCP/TLS dialing against a dead target throughout — no degradation observed", round, testDuration)
}
