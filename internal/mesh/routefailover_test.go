...
func TestRouteFailoverBetweenTwoOrigins(t *testing.T) {
	...
	if !waitUntil(5*time.Second, func() bool { return C.dev.hasRoute(route) && C.dev.metricOf(route) == 10 }) {
		t.Fatalf("C's OS route table should show metric 10; got %d", C.dev.metricOf(route))
	}
	...
	if !waitUntil(10*time.Second, func() bool { return C.dev.hasRoute(route) && C.dev.metricOf(route) == 20 }) {
		t.Errorf("C's OS route table did not update to B's metric 20; got %d, hasRoute=%v", C.dev.metricOf(route), C.dev.hasRoute(route))
	}
