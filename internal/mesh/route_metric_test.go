...
func TestRouteMetricPropagatesAndUpdates(t *testing.T) {
	...
	// shows up in `ip route`, and is the whole point of installing it.
	if !waitUntil(5*time.Second, func() bool { return B.dev.hasRoute(route) && B.dev.metricOf(route) == 7 }) {
		t.Fatalf("B's device should hold route with metric 7; got metric %d hasRoute=%v",
			B.dev.metricOf(route), B.dev.hasRoute(route))
	}
	...
	if !waitUntil(5*time.Second, func() bool { return B.dev.metricOf(route) == 3 }) {
		t.Fatalf("B's device metric should update to 3; got %d", B.dev.metricOf(route))
	}
}
